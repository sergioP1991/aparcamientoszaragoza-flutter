...
class MyGaragesScreen extends ConsumerWidget {
  static const routeName = '/my-garages';
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          AppLocalizations.of(context)!.myPropertiesTitle,
      ...
      body: homeDataState.when(
        data: (data) {
          ...
          final myGarages = d.listGarajes.where((g) => g.propietario == d.user?.uid).toList();
          ...
          if (myGarages.isEmpty) {
            return Center(
              child: Column(
                ...
                children: [
                  Icon(Icons.home_work_outlined, size: 80, color: Colors.white.withOpacity(0.1)),
                  const SizedBox(height: 16),
                  Text(
                    AppLocalizations.of(context)!.noMyGarages,
          ...
          return ListView.builder(
            ...
            itemCount: myGarages.length,
            itemBuilder: (context, index) {
              return GarageCard(
                item: myGarages[index],
                user: d.user,
              );
            },
          );
